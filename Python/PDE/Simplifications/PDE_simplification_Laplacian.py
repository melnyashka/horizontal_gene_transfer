...
def Next_Generation_PDE(f,parameters):
    ...
    #transfer_part=0
    new_f = np.maximum(0,f + dT/parameters['eps']*(f*(b_r-death_term+ transfer_part)+Laplacian_f))
    return new_f
...
figure.savefig(str("Figures/Laplacian/plot_" + str(current_time)[0:8].replace(':','_')+".pdf")) # Possibly different delimeter on Linux and Windows!
