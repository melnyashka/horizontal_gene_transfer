...
#    parameters_HJ['T_max']=int(parameters_HJ['T_max']*parameters_HJ['eps'])
    ...
    def Next_Generation_AP(u, rho, parameters_HJ, pre_init_values):
        ...
        X_large=np.arange(2*X_min,2*X_max,dX)
        transfer_kernel=  parameters_HJ['tau']*1/np.pi*(np.arctan(np.divide(X_large,parameters_HJ['delta']))-np.arctan(-np.divide(X_large,parameters_HJ['delta'])))*dX
    ...
    #    m_sq = mut_kern/cste*np.ones([len(X)])
        ...
        H = parameters_HJ['b_r']*np.exp(flux - np.power(np.ones([len(X)])*Yx,2)/(2*parameters_HJ['sigma']**2))/cste #*m_sq #*m(x) add a gaussian mutation kernel!
        H = np.sum(H,axis=0)*dy # that's the birth term
    
        A = u + dT*(-Death + H + T) # that's the birth-death-transfer term
        max_A = np.max(A)
    ...
    for i in range(nT-1):
        ...
        if i%1000==0: 
            print('HJ: i='+str(i/nT*100)+'% complete.')
    
    nT2= int(parameters['T_max']/parameters['dT'])
    for i in range(nT2):
        i2=int(i/parameters_HJ['eps'])
        A=np.max(U[i2])
        U_H[i]=np.exp(A/parameters_HJ['eps'])*np.exp((U[i2]-A)/parameters_HJ['eps'])
    
    
    #U_H=np.exp(U/parameters_HJ['eps'])
    ...
    for i in range(nbr_iteration-1):
        if i%1000==0:
            print('STOCH: '+str(i/nbr_iteration*100)+'% complete.')
        X=Next_Generation(X, parameters_S)
        U_S[i+1]=np.histogram(X,bins,rg,False)[0]/(parameters_S['K']*parameters_S['dX'])
    ...
    for i in range(nT-1):
        ...
        if i%5000==0:
            print('PDE: T= '+str(i/nT*100)+'% complete', flush = True)
    ...
    for k, v in parameters.items():
        if k == 'N0' or k == 'b_r' or k=='X_min' or k=='x_mean0': 
            smth = ",\n" 
        else: 
            smth = ", "
    ...
    l1.set_xdata(xData)
    fig.legend(loc=5,fontsize='large')
    
    #plt.xlabel(par_str )
    #plt.title(par_str)
    plt.xlim(parameters_HJ['X_min'],parameters_HJ['X_max'])
    #fig1.set_title(par_str)
    plt.suptitle(par_str)
    ...
    #plt.title(par_str,loc='right')
    
    current_time = datetime.now().time()
    
    # let's write to the file!
    with writer.saving(fig, 'Movies/plot_' + str(current_time)[0:8].replace(':','_')+'.mp4', dpi=100):
        for i in range(frameNumber-1):
            if i%50==0:
                print('Plot: '+str(i/frameNumber*100)+'% complete.')
            x1, y1 = frames1[i]
            l1.set_data( x1, y1)
