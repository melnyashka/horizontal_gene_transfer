...
for i in range(len(tau_i)):
	...
	path_to_dir = "/scratch/gene/horizontal_gene_transfer/Mesocentre/Figures/HJ/"
	build_and_save_HJ(u, parameters, pre_init_values, path = path_to_dir)
	np.savetxt(str(path_to_dir+"U_"+str(current_time)[0:8]+".txt"), U, delimiter = ',')
	np.savetxt(str(path_to_dir+"Rho_"+str(current_time)[0:8]+".txt"), Rho, delimiter = ',')
